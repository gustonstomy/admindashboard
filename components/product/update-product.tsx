...
export default function UpdateProductPage({ id }: { id: string }) {
  const { data: allProductData, isPending: allProductDataPending } =
    useGetProductDetails(Number(id));

  const productData: Product = allProductData?.data?.data;
  ...
  }, [allProductData?.data?.data, productData]);

  const [images, setImages] = useState<ImageInput[]>(
    productData?.images?.map((img) => ({
      image: null, // File objects aren't available from URL strings
  ...
  // Update images when productData changes
  useEffect(() => {
  ...
  const { mutate, isSuccess, isPending } = useUpdateProduct(id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Payload to create product:", payload);
